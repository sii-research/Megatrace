...
# -*- coding: utf-8 -*-
"""
LSTM-based Time Series Analysis for Distributed Training Logs
- Analyze continuous opCount sequences for each rank
...
class LSTMAnalyzer:
    ...
    def __init__(self, sequence_length: int = 10, hidden_size: int = 64, 
                 ...
                 epochs: int = 20, device: Optional[str] = None):
        self.sequence_length = sequence_length
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.threshold = threshold
        # Training epochs (smaller default to avoid long runs)
        self.epochs = epochs
        # Select device based on user preference
        if device == 'cpu':
    ...
    def _train_lstm(self, data: np.ndarray, epochs: Optional[int] = None) -> List[float]:
        ...
        # Resolve epochs from instance default
...
class MultiThreadedLSTMAnalyzer:
    ...
    def __init__(self, sequence_length: int = 10, hidden_size: int = 64, 
                 num_layers: int = 2, threshold: float = 2.0, max_workers: Optional[int] = None,
                 epochs: int = 10, device_per_thread: str = 'cpu'):
        self.sequence_length = sequence_length
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.threshold = threshold
        # Limit workers to a sane default to avoid oversubscription
        self.max_workers = max_workers if max_workers is not None else max(1, min(8, (os.cpu_count() or 4)))
        # Per-thread training config
        self.epochs = epochs
        # Using CUDA from many Python threads can hang; default to CPU per thread
        self.device_per_thread = device_per_thread
    ...
    def analyze_rank_sequences(self, rank: int, sequences: List[ContinuousSequence]) -> List[AnomalyResult]:
        ...
        rank_results = []
        total = len(sequences)
        print(f"[Worker] Rank {rank}: analyzing {total} sequences...", flush=True)
        ...
        for idx, sequence in enumerate(sequences):
            ...
            # Periodic progress
            if (idx + 1) % 200 == 0:
                print(f"[Worker] Rank {rank}: {idx + 1}/{total} sequences done", flush=True)
    ...
    def analyze_logs_parallel(self, rank_entries: Dict[int, List[LogEntry]]) -> Dict[int, List[AnomalyResult]]:
        ...
        print("Detecting continuous sequences...", flush=True)
        
        # Detect sequences first
        detector = ContinuousSequenceDetector()
        sequences = detector.detect_sequences(rank_entries)
        
        print(f"Found {len(sequences)} continuous sequences", flush=True)
        ...
        print(f"Processing {len(rank_sequences)} ranks with multi-threading (max_workers={self.max_workers})...", flush=True)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_rank):
                ...
                try:
                    ...
                    print(f"✓ Rank {rank} completed: {total_sequences} sequences, {anomalies_found} anomalies", flush=True)
                    
                except Exception as e:
                    print(f"✗ Error analyzing Rank {rank}: {e}", flush=True)
                    with self.results_lock:
...
class LSTMTimeSeriesAnalyzer:
    ...
    def __init__(self, sequence_length: int = 10, threshold: float = 2.0, 
                 max_workers: Optional[int] = None, use_multithreading: bool = True,
                 epochs: int = 10, device: str = 'auto'):
        ...
        if use_multithreading:
            self.multi_threaded_analyzer = MultiThreadedLSTMAnalyzer(
                ...
                epochs=epochs,
                # Default to CPU per thread to avoid CUDA thread contention
                device_per_thread=('cpu' if device == 'cuda' or device == 'auto' else device)
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LSTM Time Series Analysis for Distributed Training Logs')
    ...
    parser.add_argument('--device', choices=['auto', 'cpu', 'cuda'], default='auto', help='Device for LSTM (auto/cpu/cuda)')
    parser.add_argument('--epochs', type=int, default=10, help='Training epochs per sequence (smaller is faster)')
